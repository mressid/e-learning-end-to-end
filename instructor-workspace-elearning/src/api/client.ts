...
// Deliberately not the admin dashboard's key names. This app stores a platform
// token (`typ=user`), which the backend refuses on /admin/** just as it refuses
// an administrator's here. The two apps are separate origins so they do not in
// fact share a localStorage — but naming the keys apart is what makes that a
// decision rather than a coincidence, and it is what stops a copied token from
// one app silently 403ing every call in the other.
export const ACCESS_TOKEN_KEY = "lernova_instructor_access_token";
export const REFRESH_TOKEN_KEY = "lernova_instructor_refresh_token";
...
// One refresh at a time. Refresh tokens rotate on use and a reused one is
