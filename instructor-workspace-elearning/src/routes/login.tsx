...
import { useAuth, hasUserSession } from "@/lib/auth";
...
export const Route = createFileRoute("/login")({
   ...
   * server-rendered; on an app entirely behind a sign-in, that buys nothing
   * worth a guard that only pretends to work.
   */
  ssr: false,
  /** The mirror of the `_authenticated` guard: a session belongs in the workspace. */
  beforeLoad: () => {
    if (hasUserSession()) throw redirect({ to: "/" });
  },
  head: () => ({
    meta: [
      { title: "Sign in — Lernova for Instructors" },
      { name: "description", content: "Sign in to author and manage the courses you teach." },
      { property: "og:title", content: "Sign in — Lernova for Instructors" },
      {
        property: "og:description",
        content: "Sign in to author and manage the courses you teach.",
      },
    ],
  }),
  component: LoginPage,
    ...
    <main className="relative flex min-h-screen w-full flex-col bg-background text-foreground transition-colors duration-200 lg:grid lg:grid-cols-12">
      ...
      <div className="flex flex-1 flex-col justify-center px-4 py-12 sm:px-6 md:px-12 lg:col-span-6 lg:px-16 xl:col-span-5">
        <div className="mx-auto w-full max-w-md">
          ...
          {/*
            Where the demo-credentials filler used to be. It pasted in the seeded
            super admin, which lives in `admin_users` and cannot authenticate on
            this side at all — a button whose only possible outcome was "invalid
            credentials". The card keeps its place and says the thing people
            actually arrive here confused about.
          */}
          <div className="mt-6 flex items-start gap-2.5 rounded-xl border border-border bg-card p-3">
            <Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" />
            <p className="text-xs text-muted-foreground">{t("auth.wrongApp")}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {/* Not a link. There is no self-service reset an instructor can
                    rely on — the account may have no reachable address, which is
                    why an administrator sets the password directly. */}
                <span className="text-xs font-medium text-muted-foreground">
                  {t("auth.forgotPassword")} {t("auth.contactAdmin")}
                </span>
              </div>
              <div className="relative mt-1.5">
                <Lock
          ...
          <div className="mt-6">
            ...
            {/* Kept because the layout is built around them, disabled because
                the backend has no SSO. Buttons that silently do nothing are
                worse than buttons that say they cannot. */}
            <div className="mt-4 grid grid-cols-2 gap-3">
              <button
                type="button"
                disabled
                title={t("auth.ssoUnavailable")}
                className="inline-flex h-9 cursor-not-allowed items-center justify-center gap-2 rounded-xl border border-input bg-card text-xs font-semibold text-foreground opacity-50"
              >
      ...
      <div className="relative hidden lg:col-span-6 xl:col-span-7 lg:flex flex-col justify-between overflow-hidden border-l border-border bg-secondary/30 p-12">
        ...
        <div className="relative z-10 flex items-center justify-between">
          ...
          {/* A "public catalog" link used to sit here pointing at "/". In this
              app that is the authenticated home, so the guard would bounce it
        ...
        <div className="relative z-10 my-auto max-w-lg space-y-6">
          ...
          {/* Metric Badges */}
          {/* Three invented metrics used to sit here — student counts, a
              completion rate, an uptime figure. A sign-in screen knows none of
              them, and numbers nobody measured are worse than no numbers. Same
              grid, same weight, things that are actually true. */}
          ...
          {/* Was a "live class starting now" card naming a cohort and an
              instructor who do not exist. */}
          <div className="rounded-2xl border border-border bg-card/90 p-4 backdrop-blur shadow-sm space-y-2.5">
            ...
            <p className="text-xs text-muted-foreground">{t("auth.previewBody")}</p>
          </div>
        </div>
